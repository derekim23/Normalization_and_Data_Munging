...
#Draw in VO2Max, HGB, metabolites, etc. data
vo2path <- "/Volumes/GoogleDrive-115111199924997198421/My Drive/Objectives/Code/"
vo2_file <- "DARPA-MBA-VO2max-InBody-Spring21-08142021-AA.xlsx"
...
cog <- read_excel(paste(hgb_path, "DARPA-MBA-AMS-Spring-01052022-dk.xlsx", sep = ""), sheet = "Sheet2")

metab_file <- 'Preprocessed_DARPA-MBA-UntargetedMetabolomics-ReNormalizedData-Spring21-09272021-AA-DK.xlsx'
...
df[,c('trp/phen1', 'trp/leuc1', 'trp/isol1','trp/vali1')] <- 
  exp(df[,'565.x']-df[,c('460.x', '397.x', '376.x','566.x')]) #df[,'565.x']/df[,c('460.x', '397.x', '376.x','566.x')]
df[,'trp/(t + p + l + i + v)1'] <- 
  exp(df[,'565.x'])/apply(exp(df[,c('460.x', '397.x', '376.x','566.x')]),1,sum) #df[,'565.x']/apply(df[,c('460.x', '397.x', '376.x','566.x')],1,sum)
df[,'trp/tyr1'] <- exp(df[,'565.x'])/exp(df[,'815.x']) #df[,'565.x']/df[,'815.x']

df[,c('trp/phen2', 'trp/leuc2', 'trp/isol2','trp/vali2')] <- 
  exp(df[,'565.y']-df[,c('460.y', '397.y', '376.y','566.y')]) #df[,'565.y']/df[,c('460.y', '397.y', '376.y','566.y')]
df[,'trp/(t + p + l + i + v)2'] <- 
  exp(df[,'565.y'])/apply(exp(df[,c('460.y', '397.y', '376.y','566.y')]),1,sum) #df[,'565.y']/apply(df[,c('460.y', '397.y', '376.y','566.y')],1,sum)
df[,'trp/tyr2'] <- exp(df[,'565.y'])/exp(df[,'815.y']) #df[,'565.y']/df[,'815.y']

df[,'glut/crea1'] <- exp(df[,'100001253.x'])/exp(df[,'1221.x']) #df[,'100001253.x']/df[,'1221.x']

df[,'glut/crea2'] <- exp(df[,'100001253.y'])/exp(df[,'1221.y']) #df[,'100001253.y']/df[,'1221.y']
...
df$`subject Military Record Brief (Average MPSC)` <- as.numeric(df$`subject Military Record Brief (Average MPSC)`)
df$APS_Cum <- as.numeric(df$APS_Cum)
df$GPA_Cum <- as.numeric(df$GPA_Cum)
df$CPS <- as.numeric(df$CPS)
df$`Whole Candidate Score` <- as.numeric(df$`Whole Candidate Score`)
df$`Physical Program Score- 2021 (PPS Cum)` <- as.numeric(df$`Physical Program Score- 2021 (PPS Cum)`)
...
#Create a matrix of hypothesis test results instead this time
stats <- matrix(NA, nrow = 264, ncol = 18)
colnames(stats) <- c("HMDB", "Chem_ID", "Name", "Pre_p_Val", "Pre_t_Val", 
                     "Post_Cohen", "Post_FWER", "Post_FDR", "Post_p_Val", 
...
stats5 <- matrix(NA, nrow = 264, ncol = 8)

for (i in 1:264){
  stats5[i,1] <- short_list$HMDB[grep(chem_list[i],short_list$CHEM_ID)[1]]
  stats5[i,2] <- chem_list[i]
  stats5[i,3] <- short_list$NAME[grep(chem_list[i],short_list$CHEM_ID)[1]]
  err_x <- tryCatch({
    stats5[i,4] <- t.test(df[df$Dynamic==2,paste(chem_list[i],".x",sep="")],
                          df[df$Dynamic==1,paste(chem_list[i],".x",sep="")], paired = F)$p.value
    stats5[i,5] <- signif(t.test(df[df$Dynamic==2,paste(chem_list[i],".x",sep="")],
                                 df[df$Dynamic==1,paste(chem_list[i],".x",sep="")], paired = F)$statistic,2)
    stats5[i,6] <- signif(cohen.d(df[,paste(chem_list[i],".x",sep="")], df$Dynamic)$cohen.d[2],2)
  },
  error = function(w) w)
  if(inherits(err_x,"warning"))
    errors_x <- c(errors_x,i)
}
colnames(stats5) <- c("HMDB", "Chem_ID", "Name", "p_Val", "t_Val", 
                      "Cohen", "FWER", "FDR")

five <- c('HMDB0000715', 'HMDB0000929', 'HMDB0000158', 'HMDB0000232', 'HMDB0001067', 'HMDB0002243')

write.csv(df$`1022.x`[df$Dynamic==2],'/Users/Derek/Downloads/high.csv')
write.csv(df$`1022.x`[df$Dynamic==1],'/Users/Derek/Downloads/low.csv')


stats5[,7] <- p.adjust(as.numeric(stats5[,4]),"bonferroni")
stats5[,8] <- p.adjust(as.numeric(stats5[,4]),"fdr")
stats5[,4] <- signif(as.numeric(stats5[,4]),2)
stats5[,7] <- signif(as.numeric(stats5[,7]),2)
stats5[,8] <- signif(as.numeric(stats5[,8]),2)

write.csv(stats5, "/Users/Derek/Downloads/five_p2.csv")

ath_data <- read_excel('/Users/Derek/Downloads/khelaifi_2018_HEHP_MEMP_HEMP.xlsx', sheet = 'log_trans_data')
temp <- ath_data$Prof
ath_data <- as.matrix(ath_data[,stats5[,2]])
ath_data

stats6 <- matrix(NA, nrow = 6, ncol = 8)
temp2 <- temp

temp <- colnames(ath_data)

for (i in 1:6){
  stats6[i,1] <- short_list$HMDB[grep(temp[i],short_list$CHEM_ID)[1]]
  stats6[i,2] <- temp[i]
  stats6[i,3] <- short_list$NAME[grep(temp[i],short_list$CHEM_ID)[1]]
  err_x <- tryCatch({
    stats6[i,4] <- t.test(ath_data[temp2=="HE",paste(temp[i],sep="")],
                          ath_data[temp2=="ME",paste(temp[i],sep="")], paired = F)$p.value
    stats6[i,5] <- signif(t.test(ath_data[temp2=="HE",paste(temp[i],sep="")],
                                 ath_data[temp2=="ME",paste(temp[i],sep="")], paired = F)$statistic,2)
    stats6[i,6] <- signif(cohen.d(ath_data[,paste(temp[i],sep="")], temp3)$cohen.d[2],2)
  },
  error = function(w) w)
  if(inherits(err_x,"warning"))
    errors_x <- c(errors_x,i)
...
colnames(stats6) <- c("HMDB", "Chem_ID", "Name", "p_Val", "t_Val", 
                      "Cohen", "FWER", "FDR")

stats6[,7] <- p.adjust(as.numeric(stats6[,4]),"bonferroni")
stats6[,8] <- p.adjust(as.numeric(stats6[,4]),"fdr")
stats6[,4] <- signif(as.numeric(stats6[,4]),2)
stats6[,7] <- signif(as.numeric(stats6[,7]),2)
stats6[,8] <- signif(as.numeric(stats6[,8]),2)

write.csv(stats6,'/Users/Derek/Downloads/ay.csv')
...
lm_mpsc_kyn_trp <- lm(`subject Military Record Brief (Average MPSC)`~`kyn/trp1`,
                      data=df[df$`kyn/trp1`<60,])
slm_mpsc_kyn_trp <- summary(lm_mpsc_kyn_trp)

lm_cps_kyna_trp <- lm(CPS~`kyna/trp1`,
                      data=df[df$`kyna/trp1`<60,])
slm_cps_kyna_trp <- summary(lm_cps_kyna_trp)

lm_aps_kyn_trp <- lm(APS_Cum~`kyn/trp1`,
                      data=df[df$`kyn/trp1`<60,])
slm_aps_kyn_trp <- summary(lm_aps_kyn_trp)
...
plot(x=df[df$`kyn/trp1`<60,]$`kyn/trp1`, 
     y=as.numeric(df[df$`kyn/trp1`<60,]$APS_Cum),
     xlab = "Pre-Stress Kynurenine/Tryptophan",
     ylab = "Academic Performance Score",
     main = "Academic Performance Score vs. 
     Segment 2 Kynurenine/Tryptophan",
     cex.lab = 1.4,
     cex.main = 1.4,
     ylim =c(1.5, 5.2))
text(0.39, 5.1, 
     bquote("Correlation" ==
              .(round(cor(df[df$`kyn/trp1`<60,]$`kyn/trp1`,
                          as.numeric(df[df$`kyn/trp1`<60,]$APS_Cum), 
                          use = 
                            "pairwise.complete.obs"),2))), 
     pos = 4)
text(0.39, 4.84, bquote(R^2 == .(round(slm_aps_kyn_trp$r.squared,3))), pos = 4)
text(0.39, 4.58, bquote("p-value" == .(0.74)), pos = 4)
abline(lm_aps_kyn_trp, lty=2, col = "red", lwd = 2)
     ...
     bquote("Correlation" ==
              .(round(cor(df[df$`kyn/trp1`<60,]$`kyn/trp1`,
                          ...
                          use = 
                            "pairwise.complete.obs"),2))), 
     pos = 4)
text(0.25, 4.84, bquote(R^2 == .(round(slm_aps_kyn_trp$r.squared,3))), pos = 4)
